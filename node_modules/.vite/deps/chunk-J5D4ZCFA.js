...
var B = (e, t) => $e(e, Fe(t));
function Ge(e) {
  return B(x({}, e), { level: e?.level || Z.level });
}
function Q(e, t, r = S) {
  return e[r] = t, e;
}
...
var BlockchainApiController = {
  ...
  async isNetworkSupported(networkId) {
    ...
    try {
      if (!state3.supportedChains.http.length) {
        await BlockchainApiController.getSupportedNetworks();
      }
    } catch (e) {
      return false;
    }
    return state3.supportedChains.http.includes(networkId);
  },
  async getSupportedNetworks() {
    try {
      ...
      state3.supportedChains = supportedChains;
      return supportedChains;
    } catch {
      return state3.supportedChains;
    }
  },
  ...
  async fetchTokenPrice({ addresses, caipNetworkId = ChainController.state.activeCaipNetwork?.caipNetworkId }) {
    ...
    const result = await state3.api.post({
  ...
  setClientId(clientId) {
    ...
    state3.api = new FetchUtil({ baseUrl, clientId });
  }
};
...
var TelemetryController = {
  ...
  async sendError(error, category) {
    ...
    const now = Date.now();
    const recentErrors = state4.events.filter((event) => {
...
var controller2 = {
  ...
  setChainImage(key, value) {
    state5.chainImages[key] = value;
  },
  setConnectorImage(key, value) {
    state5.connectorImages = { ...state5.connectorImages, [key]: value };
  },
...
var AssetUtil = {
  ...
  async fetchNetworkImage(imageId) {
    ...
    if (!state6.networkImagePromises[imageId]) {
      state6.networkImagePromises[imageId] = ApiController._fetchNetworkImage(imageId);
    }
    await state6.networkImagePromises[imageId];
  ...
  async fetchTokenImage(imageId) {
    ...
    if (!state6.tokenImagePromises[imageId]) {
      state6.tokenImagePromises[imageId] = ApiController._fetchTokenImage(imageId);
    }
    await state6.tokenImagePromises[imageId];
...
var EventsController = {
  ...
  shouldFlushEvents() {
    const isOverMaxSize = JSON.stringify(state7.pendingEvents).length / 1024 > MAX_PENDING_EVENTS_KB;
    const isExpired = state7.lastFlush + FLUSH_EVENTS_INTERVAL_MS < Date.now();
  ...
  _setPendingEvent(payload) {
    try {
      ...
      state7.reportedErrors["FORBIDDEN"] = false;
  ...
  sendEvent(data) {
    ...
    if (OptionsController.state.features?.analytics || MANDATORY_EVENTS.includes(data.event)) {
      EventsController._setPendingEvent(state7);
    }
  ...
  sendWalletImpressionEvent(item) {
    state7.walletImpressions.push(item);
  },
  ...
  _submitPendingEvents() {
    state7.lastFlush = Date.now();
    if (state7.pendingEvents.length === 0 && state7.walletImpressions.length === 0) {
      return;
    }
    try {
      const batch = EventsController._transformPendingEventsForBatch(state7.pendingEvents);
      if (state7.walletImpressions.length) {
        batch.push({
          eventId: CoreHelperUtil.getUUID(),
          url: window.location.href,
          domain: window.location.hostname,
          ...
          props: {
            ...
            items: [...state7.walletImpressions]
          }
        });
      }
      api2.sendBeacon({
      ...
      state7.reportedErrors["FORBIDDEN"] = false;
      state7.pendingEvents = [];
      state7.walletImpressions = [];
    } catch (err) {
      state7.reportedErrors["FORBIDDEN"] = true;
    }
  },
  subscribeToFlushTriggers() {
    if (state7.subscribedToVisibilityChange) {
      return;
    }
    if (typeof document === "undefined") {
      return;
    }
    state7.subscribedToVisibilityChange = true;
...
var ApiController = {
  ...
  async prefetchWalletRanks() {
    ...
    state8.explorerWallets = data;
    ConnectorController.extendConnectorsWithExplorerWallets(data);
    const caipNetworkIds = ChainController.getRequestedCaipNetworkIds().join(",");
    state8.explorerFilteredWallets = data.filter((wallet) => wallet.chains?.some((chain) => caipNetworkIds.includes(chain)));
  },
  async fetchFeaturedWallets() {
    ...
    if (featuredWalletIds?.length) {
      ...
      state8.featured = sortedData;
      state8.allFeatured = sortedData;
    }
  },
  async fetchRecommendedWallets() {
    try {
      state8.isFetchingRecommendedWallets = true;
      ...
      state8.allRecommended = data;
      state8.count = count ?? 0;
    } catch {
    } finally {
      state8.isFetchingRecommendedWallets = false;
    }
  },
  async fetchWalletsByPage({ page }) {
    ...
    state8.mobileFilteredOutWalletsLength = mobileFilteredOutWalletsLength + (state8.mobileFilteredOutWalletsLength ?? 0);
    const images = data.slice(0, imageCountToFetch).map((w2) => w2.image_id).filter(Boolean);
    await Promise.allSettled(images.map((id) => ApiController._fetchWalletImage(id)));
    state8.wallets = CoreHelperUtil.uniqueBy([...state8.wallets, ...ApiController._filterOutExtensions(data)], "id").filter((w2) => w2.chains?.some((chain) => chains.includes(chain)));
  ...
  async initializeExcludedWallets({ ids }) {
    ...
    if (data) {
      data.forEach((wallet) => {
        state8.excludedWallets.push({ rdns: wallet.rdns, name: wallet.name });
      });
    }
  },
  async searchWallet({ search, badge }) {
    ...
    state8.search = ApiController._filterOutExtensions(data);
  },
  initPromise(key, fetchFn) {
    const existingPromise = state8.promises[key];
    if (existingPromise) {
      return existingPromise;
    }
    return state8.promises[key] = fetchFn();
  },
  ...
  filterByNamespaces(namespaces) {
    if (!namespaces?.length) {
      state8.featured = state8.allFeatured;
      state8.recommended = state8.allRecommended;
      return;
    }
    const caipNetworkIds = ChainController.getRequestedCaipNetworkIds().join(",");
    state8.featured = state8.allFeatured.filter((wallet) => wallet.chains?.some((chain) => caipNetworkIds.includes(chain)));
    state8.recommended = state8.allRecommended.filter((wallet) => wallet.chains?.some((chain) => caipNetworkIds.includes(chain)));
    state8.filteredWallets = state8.wallets.filter((wallet) => wallet.chains?.some((chain) => caipNetworkIds.includes(chain)));
  },
  clearFilterByNamespaces() {
    state8.filteredWallets = [];
  },
...
var controller3 = {
  ...
  push(view, data) {
    ...
    if (finalView !== state9.view) {
      state9.view = finalView;
      state9.history.push(finalView);
      state9.data = finalData;
    }
  },
  reset(view, data) {
    state9.view = view;
  ...
  goBack() {
    ...
    if (state9.data?.wallet) {
      state9.data.wallet = void 0;
    }
    if (state9.data?.redirectView) {
      state9.data.redirectView = void 0;
    }
    setTimeout(() => {
      if (shouldReload) {
...
var controller6 = {
  ...
  getEnabledNamespaces() {
    return Object.entries(state13.filterByNamespaceMap).filter(([_, enabled]) => enabled).map(([namespace]) => namespace);
  },
  ...
  getAuthConnector(chainNamespace) {
    ...
    const authConnector = state13.connectors.find((c) => c.id === ConstantsUtil.CONNECTOR_ID.AUTH);
    if (!authConnector) {
      return void 0;
    }
  ...
  getAnnouncedConnectorRdns() {
    return state13.connectors.filter((c) => c.type === "ANNOUNCED").map((c) => c.info?.rdns);
  },
  getConnectorById(id) {
    const sortedConnectors = ConnectorUtil.sortConnectorsByPriority(state13.allConnectors);
  ...
  getConnector({ id, namespace }) {
    ...
    const connectorsByNamespace = state13.allConnectors.filter((c) => c.chain === namespaceToUse);
  ...
  extendConnectorsWithExplorerWallets(explorerWallets) {
    state13.allConnectors.forEach((connector) => {
...
var AdapterController = {
  state: state16,
  initialize(adapters) {
    state16.adapters = { ...adapters };
  },
  get(namespace) {
    return state16.adapters[namespace];
  }
};
...
var ProviderController = {
  ...
  subscribeProviders(callback) {
    return subscribe(state17.providers, () => callback(state17.providers));
  },
  setProvider(chainNamespace, provider) {
    if (chainNamespace && provider) {
      state17.providers[chainNamespace] = ref(provider);
    }
  },
  getProvider(chainNamespace) {
    ...
    return state17.providers[chainNamespace];
  },
  setProviderId(chainNamespace, providerId) {
    if (providerId) {
      state17.providerIds[chainNamespace] = providerId;
    }
  },
  getProviderId(chainNamespace) {
    ...
    return state17.providerIds[chainNamespace];
  },
  reset() {
    state17.providers = { ...CLEAN_PROVIDERS_STATE };
    state17.providerIds = { ...CLEAN_PROVIDERS_STATE };
  },
  resetChain(chainNamespace) {
    state17.providers[chainNamespace] = void 0;
    state17.providerIds[chainNamespace] = void 0;
  }
};
...
var controller9 = {
  ...
  setNetworkBalanceInUsd(networkBalanceInUSD) {
    state18.networkBalanceInUSD = networkBalanceInUSD;
  },
  setLoading(loading) {
  ...
  getSdkEventProperties(error) {
    return {
      ...
      token: state18.token?.symbol || "",
      amount: state18.sendTokenAmount ?? 0,
      network: ChainController.state.activeCaipNetwork?.caipNetworkId || ""
    };
  },
  async sendToken() {
  ...
  async fetchTokenBalance(onError) {
    ...
    try {
      if (address && chainId && chain) {
        const balances = await BalanceUtil.getMyTokensWithBalance();
        state18.tokenBalances = balances;
  ...
  fetchNetworkBalance() {
    if (state18.tokenBalances.length === 0) {
      return;
    }
    const networkTokenBalances = SwapApiUtil.mapBalancesToSwapTokens(state18.tokenBalances);
...
var controller10 = {
  ...
  subscribeAccountStateProp(property, callback, chain) {
    const activeChain = chain || state19.activeChain;
    if (!activeChain) {
      return () => void 0;
    }
    return subscribeKey(state19.chains.get(activeChain)?.accountState || {}, property, callback);
  },
  subscribeChainProp(property, callback, chain) {
    let prev = void 0;
    return subscribe(state19.chains, () => {
      const activeChain = chain || state19.activeChain;
      if (activeChain) {
        const nextValue = state19.chains.get(activeChain)?.[property];
        if (prev !== nextValue) {
          prev = nextValue;
          callback(nextValue);
        }
  ...
  initialize(adapters, caipNetworks, clients) {
    ...
    if (adapters?.length === 0 || !adapterToActivate) {
      state19.noAdapters = true;
    }
    if (!state19.noAdapters) {
      state19.activeChain = adapterToActivate?.namespace;
      state19.activeCaipNetwork = activeCaipNetwork;
      ChainController.setChainNetworkData(adapterToActivate?.namespace, {
        caipNetwork: activeCaipNetwork
      });
      if (state19.activeChain) {
  ...
  removeAdapter(namespace) {
    if (state19.activeChain === namespace) {
      const nextAdapter = Array.from(state19.chains.entries()).find(([chainNamespace]) => chainNamespace !== namespace);
      if (nextAdapter) {
    ...
    state19.chains.delete(namespace);
  },
  addAdapter(adapter, { connectionControllerClient }, caipNetworks) {
    ...
    state19.chains.set(adapter.namespace, {
  ...
  addNetwork(network) {
    const chainAdapter = state19.chains.get(network.chainNamespace);
    if (chainAdapter) {
      const newNetworks = [...chainAdapter.caipNetworks || []];
      if (!chainAdapter.caipNetworks?.find((caipNetwork) => caipNetwork.id === network.id)) {
        newNetworks.push(network);
      }
      state19.chains.set(network.chainNamespace, { ...chainAdapter, caipNetworks: newNetworks });
  ...
  removeNetwork(namespace, networkId) {
    const chainAdapter = state19.chains.get(namespace);
    if (chainAdapter) {
      const isActiveNetwork = state19.activeCaipNetwork?.id === networkId;
      ...
      state19.chains.set(namespace, { ...chainAdapter, caipNetworks: newCaipNetworksOfAdapter });
  ...
  setAdapterNetworkState(chain, props) {
    const chainAdapter = state19.chains.get(chain);
    if (chainAdapter) {
      chainAdapter.networkState = {
        ...chainAdapter.networkState || networkState,
        ...props
      };
      state19.chains.set(chain, chainAdapter);
    }
  },
  setChainAccountData(chain, accountProps, _unknown = true) {
    ...
    if (chainAdapter) {
      ...
      state19.chains.set(chain, { ...chainAdapter, accountState: newAccountState });
      if (state19.chains.size === 1 || state19.activeChain === chain) {
        if (accountProps.caipAddress) {
          state19.activeCaipAddress = accountProps.caipAddress;
        }
      }
    }
  },
  setChainNetworkData(chain, networkProps) {
    ...
    if (chainAdapter) {
      ...
      state19.chains.set(chain, { ...chainAdapter, networkState: newNetworkState });
    }
  },
  // eslint-disable-next-line max-params
  ...
  setActiveNamespace(chain) {
    state19.activeChain = chain;
    const newAdapter = chain ? state19.chains.get(chain) : void 0;
    const caipNetwork = newAdapter?.networkState?.caipNetwork;
    if (caipNetwork?.id && chain) {
      state19.activeCaipAddress = newAdapter?.accountState?.caipAddress;
      state19.activeCaipNetwork = caipNetwork;
  ...
  setActiveCaipNetwork(caipNetwork) {
    ...
    const isSameNamespace = state19.activeChain === caipNetwork.chainNamespace;
    if (!isSameNamespace) {
      ChainController.setIsSwitchingNamespace(true);
    }
    const newAdapter = state19.chains.get(caipNetwork.chainNamespace);
    state19.activeChain = caipNetwork.chainNamespace;
    state19.activeCaipNetwork = caipNetwork;
    ChainController.setChainNetworkData(caipNetwork.chainNamespace, { caipNetwork });
    let address = newAdapter?.accountState?.address;
    if (address) {
      state19.activeCaipAddress = `${caipNetwork.chainNamespace}:${caipNetwork.id}:${address}`;
    } else if (isSameNamespace && state19.activeCaipAddress) {
      const { address: parsedAddress } = ParseUtil.parseCaipAddress(state19.activeCaipAddress);
      address = parsedAddress;
      state19.activeCaipAddress = `${caipNetwork.caipNetworkId}:${address}`;
    } else {
      state19.activeCaipAddress = void 0;
    }
    ChainController.setChainAccountData(caipNetwork.chainNamespace, {
      address,
      caipAddress: state19.activeCaipAddress
    });
    SendController.resetSend();
    PublicStateController.set({
      activeChain: state19.activeChain,
      selectedNetworkId: state19.activeCaipNetwork?.caipNetworkId
    });
    StorageUtil.setActiveCaipNetworkId(caipNetwork.caipNetworkId);
  ...
  addCaipNetwork(caipNetwork) {
    ...
    const chain = state19.chains.get(caipNetwork.chainNamespace);
    if (chain) {
  ...
  async switchActiveNetwork(network, { throwOnFailure = false } = {}) {
    ...
    const isAuthProvider = ProviderController.getProviderId(state19.activeChain) === "AUTH";
  ...
  getConnectionControllerClient(_chain) {
    const chain = _chain || state19.activeChain;
    if (!chain) {
  ...
  getNetworkProp(key, namespace) {
    const chainNetworkState = state19.chains.get(namespace)?.networkState;
  ...
  getRequestedCaipNetworks(chainToFilter) {
    const adapter = state19.chains.get(chainToFilter);
  ...
  getAllRequestedCaipNetworks() {
    ...
    state19.chains.forEach((chainAdapter) => {
  ...
  getActiveCaipNetwork(chainNamespace) {
    if (chainNamespace) {
      return state19.chains.get(chainNamespace)?.networkState?.caipNetwork;
    }
    return state19.activeCaipNetwork;
  },
  getActiveCaipAddress() {
    return state19.activeCaipAddress;
  },
  getApprovedCaipNetworkIds(namespace) {
    const adapter = state19.chains.get(namespace);
  ...
  checkIfSupportedNetwork(namespace, caipNetworkId) {
    const activeCaipNetworkId = caipNetworkId || state19.activeCaipNetwork?.caipNetworkId;
  ...
  checkIfSupportedChainId(chainId) {
    ...
    const requestedCaipNetworks = ChainController.getRequestedCaipNetworks(state19.activeChain);
    return requestedCaipNetworks?.some((network) => network.id === chainId);
  },
  checkIfSmartAccountEnabled() {
    const networkId = NetworkUtil.caipNetworkIdToNumber(state19.activeCaipNetwork?.caipNetworkId);
    const activeChain = state19.activeChain;
  ...
  checkIfNamesSupported() {
    const activeCaipNetwork = state19.activeCaipNetwork;
  ...
  setIsSwitchingNamespace(isSwitchingNamespace) {
    state19.isSwitchingNamespace = isSwitchingNamespace;
  },
  getFirstCaipNetworkSupportsAuthConnector() {
    const availableChains = [];
    let firstCaipNetwork = void 0;
    state19.chains.forEach((chain) => {
    ...
    if (availableChains.length > 0) {
      ...
      firstCaipNetwork = firstAvailableChain ? state19.chains.get(firstAvailableChain)?.caipNetworks?.[0] : void 0;
      return firstCaipNetwork;
    }
    return void 0;
  },
  getAccountData(chainNamespace) {
    const namespace = chainNamespace || state19.activeChain;
    if (!namespace) {
      return void 0;
    }
  ...
  getRequestedCaipNetworkIds() {
    ...
    const chains = namespace ? [state19.chains.get(namespace)] : Array.from(state19.chains.values());
  ...
  setLastConnectedSIWECaipNetwork(network) {
    state19.lastConnectedSIWECaipNetwork = network;
  },
  getLastConnectedSIWECaipNetwork() {
    return state19.lastConnectedSIWECaipNetwork;
  },
  async fetchTokenBalance(onError) {
...
//# sourceMappingURL=chunk-J5D4ZCFA.js.map
